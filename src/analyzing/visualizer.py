...
class SurvivalVisualizer:
    ...
    def plot_survival_curves(
        ...
        merged_df = ensemble_df.merge(
            processed_df[["patient_id", "time", "event"]], on="patient_id"
        )
        ...
        for i, (group_name, group_df) in enumerate(merged_df.groupby("risk_group")):
    ...
    def plot_whatif_treatment_analysis(
        ...
        Args:
            experiment_results: 包含what-if分析結果的實驗結果列表
            figsize: 圖片大小
        """
        ...
        # 計算每種治療的平均效果
        treatment_effects = (
            combined_df.groupby(["test_treatment", "stage"])["change_months"]
            .agg(["mean", "std", "count"])
            .reset_index()
        )
    ...
    def _plot_treatment_combinations(
        ...
        # 計算前10個最佳治療組合
        best_treatments = (
            combined_df.groupby("test_treatment")["change_months"]
            .mean()
    ...
    def plot_temporal_prediction_analysis(
        ...
        Args:
            processed_df: 包含真實生存資料的DataFrame
            time_bins: 時間分組數量
            figsize: 圖片大小
        """
        ...
        for file_idx, ensemble_file in enumerate(ensemble_files[:4]):  # 最多顯示4個
            ...
            # 計算每個時間段的誤差統計
            time_stats = (
                non_censored.groupby("time_bin")["prediction_error"]
                .agg(["mean", "std", "count"])
                .reset_index()
            )
